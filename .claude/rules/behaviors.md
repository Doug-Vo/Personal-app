...
**Question picker** — when a named topic is selected on the START panel, `fetchAndRenderQPicker(category, topic)` calls `GET /api/yki/questions?category=X&topic=Y` (60/min rate limit) and renders a scrollable list inside `#yki-qpicker-list`. The list is hidden when "🔀 Random" topic is selected or when `resetStartPanel()` is called. Each list item holds a `data-qidx` attribute; the first item is always "🔀 Random from this topic" (pre-selected). Questions that already have saved notes show a ✓ amber checkmark (`.yki-qpicker-done`). Clicking an item sets `uiSelectedQuestion`; clicking Start passes it as the `preloaded` argument to `enterPrep()`, bypassing the API fetch. `uiSelectedQuestion` is always cleared in `enterStart()` and `hideQPicker()`. The backing question array is stored in the closure variable `qpickerQuestions` (not on the DOM node). `GET /api/yki/questions` rejects empty topic — it is only callable for named topics; the JS enforces this by calling `hideQPicker()` instead of fetching when `uiTopic` is empty.
